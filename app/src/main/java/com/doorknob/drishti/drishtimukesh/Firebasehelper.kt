...
suspend fun hasActiveSubscription(courseId: String): Boolean {
    ...
    return try {
        ...
        // 🚫 BLOCKED → NEVER ALLOW
        if (role == "Blocked") {
            Log.d(TAG, "User blocked → access denied")
            return false
        }

        // ✅ OFFLINE / ADMIN-ADDED → ALWAYS ALLOW
        if (role == "Offline") {
            Log.d(TAG, "Offline student → bypass subscription check")
            return true
        }

        // 👇 STUDENT → NORMAL SUBSCRIPTION CHECK
        // ---------- EXISTING LOGIC ----------

        val subscriptionMap =
            userDoc.get("subscriptionExpiry") as? Map<*, *>

        val expiryFromAdmin =
            subscriptionMap?.get(courseId) as? Long

        if (expiryFromAdmin != null &&
            System.currentTimeMillis() < expiryFromAdmin
        ) {
            Log.d(TAG, "Active via subscriptionExpiry")
            return true
        }
        ...
        val latestTransaction =
            transactions.documents.maxByOrNull {
                it.getLong("timestamp") ?: 0L
            } ?: return false

        val months =
            latestTransaction.getLong("subscriptionMonths") ?: 0L
        val purchaseTime =
            latestTransaction.getLong("timestamp") ?: 0L

        val expiryTime =
            purchaseTime + months * 30L * 24 * 60 * 60 * 1000

        val isActive = System.currentTimeMillis() < expiryTime
        Log.d(TAG, "Paid subscription active: $isActive")
